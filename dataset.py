...
def make_mesh(patch_w,patch_h):
    ...
    x_flat = x_flat[np.newaxis,:]# add one dim in this place 
    y_one = np.ones(patch_h)
    y_one = y_one[:,np.newaxis]
    x_mesh = np.matmul(y_one , x_flat)#Matrix product of the two arrays
    #x_mesh = [[  0.   1.   2. ... patch_w.]
             #...
             #[  0.   1.   2. ...  patch_w.]]
...
class TrainDataset(Dataset):
    def __init__(self, data_path, exp_path, patch_w=560, patch_h=315, rho=16):
        ...
        self.imgs = open(data_path, 'r').readlines()#Train_List.txt
    ...
    def __getitem__(self, index):
        ...
        img_names = value.split(' ')#00000146/00000146_10242.jpg 00000146/00000146_10244.jpg
        #imgload and resize to 640,360
        ...
        img_1 = np.mean(img_1, axis=2, keepdims=True)  #axis=2：to get 3rd dim mean value
        #keepdims=True:  The result retains its original dim
        img_1 = np.transpose(img_1, [2, 0, 1])# 2 dim -> 0 dim  0,1 dim—>1,2 dim
        print(self.train_path + img_names[1][:-1])
        ...
        #a number of rand int from rho to the value of(self.WIDTH - self.rho - self.patch_w)
